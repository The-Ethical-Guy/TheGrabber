...
def port_scan(ip, port, timeout=1):
    ...
    sock.close()
    if result == 0:
        return (f"the port {port} is open")
    else:
        return (f"the port {port} is closed")

def main():
    try:
        num_attempts = int(input(f"{green}ENTER THE NUMBER OF IPS TO GUESS: {font}"))
        theport = int(input(f"{green}ENTER THE PORT TO SCAN: {font}"))
            ...
            with open(file_path, "w") as ip_file:
                for _ in range(num_attempts):
                    ...
                    if send_ping_request(random_ip):
                        ...
                        port_result = port_scan(random_ip, theport)
                        print(f"{green}IP: {random_ip} | STATUS: working | PORT: {port_result} | TYPE: {ip_type}{font}")
                        ip_file.write(f"IP: {random_ip} | STATUS: working | PORT: {port_result} | TYPE: {ip_type}\n")
                    else:
